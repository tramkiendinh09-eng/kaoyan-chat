...
7. breakdown 必须按题型给出具体分项：翻译用“信息准确、句法理解、中文表达”；小作文用“任务覆盖、格式语气、语言质量”；大作文用“审题内容、结构论证、语言质量”。
8. polishedAnswer 必须给出完整可参考文本：翻译题给参考译文，写作题给一版可背诵但不空泛的改写范文。
...
async function apiEnglishGrade(req, res) {
  ...
  try {
    ...
    const prompt = buildEnglishGradePrompt({
      ...
      answerStats: body.answerStats,
      studentAnswer,
      total
    });
    const raw = await callOpenAITextOnce(provider, ENGLISH_GRADING_SYSTEM_PROMPT, [{ role: 'user', content: prompt }], 2400);
...
function buildEnglishGradePrompt(input) {
  const lines = [
    ...
    `【学生作答统计】${cleanText(input.answerStats || '') || '（未提供）'}`,
